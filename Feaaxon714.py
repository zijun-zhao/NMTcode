...
for info in os.listdir('/home/penglab/Documents/Janelia_1000'):
    ...
    cur_hemi = get_axon_separately(testNeu,anofile, bs, region_used = ccfINFO.index)   
    axonlist = cur_hemi['axon']

    FeatureTb.iloc[iterrow,0:2*len(ccfINFO.index)] = axonlist.values
    axonlist1 = axonlist[0:len(ccfINFO.index)]
    axonlist2 = axonlist[len(ccfINFO.index):]
    FeatureTb.iloc[iterrow,2*len(ccfINFO.index):] = axonlist1.values+axonlist2.values
    iterrow=iterrow+1    
...
FeatureTb.to_excel('/home/penglab/Documents/dataSource/JaneliaAxonccf714.xlsx')
