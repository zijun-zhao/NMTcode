...
def single_brain_log(brain_regis, path, input_sub_brain):
    ...
    with open(os.path.join(os.path.join(os.path.join(path, b_info),input_sub_brain), 'affine_swc.bat'), 'w') as OPATH:
        OPATH.writelines(['\n',
                          ...
                          ')',
                          '\n'])
    filepath_Aff = os.path.join(os.path.join(os.path.join(path, b_info),input_sub_brain),  'affine_swc.bat')
    ...
    # directory = 'C:/'
    with open(os.path.join(os.path.join(os.path.join(path, b_info),input_sub_brain), 'warp_swc.bat'), 'w') as OPATH_2:
        OPATH_2.writelines(['\n',
                            ...
                            ')',
                            '\n'])
    filepath_Warp = os.path.join(os.path.join(os.path.join(path, b_info),input_sub_brain),  'warp_swc.bat')
    ...
    print(" Time used for warpping brain " + str(b_info) + ' is %.2f ' % sub_elapsed)
    '''
...
if __name__ == '__main__':
    ...
    exist_ccfbrain = os.path.exists(os.path.join(folder_path, 'average_template_25_u8_xpad.v3draw'))
    if not exist_ccfbrain:
        print('Make sure the CCF standard brain template exists.')
    path_ccfbrain = os.path.join(folder_path, 'average_template_25_u8_xpad.v3draw')

    ##
    exist_exe_affine = os.path.exists(
        os.path.join(os.path.join(folder_path, 'second_affine'), 'main_warp_from_affine.exe'))
    if not exist_exe_affine:
        print('Make sure the .exe to perform affinement exists')
    path_exe_affine = os.path.join(os.path.join(folder_path, 'second_affine'), 'main_warp_from_affine.exe')

    exist_manaulM = os.path.exists(os.path.join(os.path.join(folder_path, 'second_affine'), 'Manual_marker'))
    if not exist_manaulM:
        print('Make sure the folder containing all manually-labelled marker files exists.')
    path_manaulM = os.path.join(os.path.join(folder_path, 'second_affine'), 'Manual_marker')

    exist_ccfM = os.path.exists(os.path.join(os.path.join(folder_path, 'second_affine'), 'CCF_marker'))
    if not exist_ccfM:
        print('Make sure the folder containing marker inside CCF exists.')
    path_ccfM = os.path.join(os.path.join(folder_path, 'second_affine'), 'CCF_marker')

    exist_stripmove = os.path.exists(os.path.join(os.path.join(folder_path, 'second_affine'), 'stripMove'))
    if not exist_stripmove:
        print('Make sure the folder containing CCF brain of removing strips exists.')
    path_stripm = os.path.join(os.path.join(folder_path, 'second_affine'), 'stripMove')

    ##
    exist_exe_warp = os.path.exists(
        os.path.join(os.path.join(folder_path, 'third_warp_swc'), 'main_warp_from_df_new.exe'))
    if not exist_exe_warp:
        print('Make sure the .exe to perform warp exists')
    path_exe_warp = os.path.join(os.path.join(folder_path, 'third_warp_swc'), 'main_warp_from_df_new.exe')

    exist_afBrain = os.path.exists(os.path.join(os.path.join(folder_path, 'third_warp_swc'), 'affined_brain'))
    if not exist_afBrain:
        print('Make sure the folder containing affined brains exists.')
    path_af_brain = os.path.join(os.path.join(folder_path, 'third_warp_swc'), 'affined_brain')

    exist_auto_brain_m = os.path.exists(os.path.join(os.path.join(folder_path, 'third_warp_swc'), 'brain_auto_marker'))
    if not exist_auto_brain_m:
        print('Make sure the folder containing auto generated marker for all brains exists.')
    path_autoBrain_m = os.path.join(os.path.join(folder_path, 'third_warp_swc'), 'brain_auto_marker')

    exist_auto_ccf_m = os.path.exists(os.path.join(os.path.join(folder_path, 'third_warp_swc'), 'CCF_auto_marker'))
    if not exist_auto_ccf_m:
        print('Make sure the folder containing auto generated marker for CCF brains exists.')
    path_autoCCF_m = os.path.join(os.path.join(folder_path, 'third_warp_swc'), 'CCF_auto_marker')

    exist_ssd = os.path.exists(os.path.join(os.path.join(folder_path, 'third_warp_swc'), 'ssd_grid'))
    if not exist_ssd:
        print('Make sure the folder containing grid. swc generated using SSD algorithm.')
    path_ssdgrid = os.path.join(os.path.join(folder_path, 'third_warp_swc'), 'ssd_grid')
    ...
    for iter_brain in brain_folder_list:
        for iter_sub_brain in os.listdir(os.path.join(folder_path, iter_brain)):
            ...
            # single_brain_log(brain_regisTB,path,iter_brain)
    pool.close()
    pool.join()
