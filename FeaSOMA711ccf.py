...
#%%
def get_SOMA_region(testNeu, annotation, brain_structure, radiusSOMA,ccfINFO):
    #First read the file
    ...
    #    conCHILDsoma = pd.concat([child1DF, childm1DF])
        ...
        if somaDF.shape[0]>0:
            ...
            if nonezerochildm1DF.shape[0]>0:
                ...
                BR_corDF= pd.DataFrame(columns=BR_somaDF.columns)
                for ii in ccfINFO.loc[child_id]['Child ID'].split():      
                    BR_corDF=BR_corDF.append(BR_somaDF[BR_somaDF['region_id']==int(ii)],ignore_index=False)
                somaDF ['x'] = np.mean(BR_corDF['x'])
                somaDF ['y'] = np.mean(BR_corDF['y'])
                somaDF ['z'] = np.mean(BR_corDF['z'])
...
for info in os.listdir('/home/penglab/Documents/Janelia_1000'):
    ...
    somaDF = get_SOMA_region(testNeu, anofile, bs, 20,ccfINFO)
...
FeatureTb.to_excel('/home/penglab/Documents/dataSource/JaneliaSomaccf.xlsx')
...
'''for out of range file, denote it as -1
   for no soma denote as -2'''
